...
public class EntryPoint
{
    ...
    void ExportchaptersToXML(String exportFile)
    {
        ...
        try
        {
            ...
            XmlElement EditionEntry = doc.CreateElement("EditionEntry");
            root.AppendChild(EditionEntry);

            XmlElement chapter;
            foreach (Marker marker in myVegas.Project.Markers)
            {
                chapter = doc.CreateElement("ChapterAtom");

                XmlElement elem = doc.CreateElement("ChapterTimeStart");
                // it's important to set Vegas project properties to Ruler : Time (in File/Properties)
                XmlText text = doc.CreateTextNode(marker.Position.ToString().Replace(",", "."));
                elem.AppendChild(text);
                chapter.AppendChild(elem);

                XmlElement ChapterDisplay = doc.CreateElement("ChapterDisplay");
                XmlElement ChapterString = doc.CreateElement("ChapterString");
                XmlText textChapter = doc.CreateTextNode(marker.Label.ToString());
                ChapterString.AppendChild(textChapter);
                ChapterDisplay.AppendChild(ChapterString);

                XmlElement ChapterLanguage = doc.CreateElement("ChapterLanguage");
                XmlText textChapterLanguage = doc.CreateTextNode("und");
                ChapterLanguage.AppendChild(textChapterLanguage);
                ChapterDisplay.AppendChild(ChapterLanguage);

                XmlElement ChapLanguageIETF = doc.CreateElement("ChapLanguageIETF");
                XmlText textChapLanguageIETF = doc.CreateTextNode("und");
                ChapLanguageIETF.AppendChild(textChapLanguageIETF);
                ChapterDisplay.AppendChild(ChapLanguageIETF);

                chapter.AppendChild(ChapterDisplay);

                EditionEntry.AppendChild(chapter);
            }
